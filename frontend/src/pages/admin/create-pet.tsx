...
const CreatePetPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await axios.post(`${config.apiBaseUrl}/pet_private`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
      setNotification({ message: 'Tạo pet thành công', type: 'success' });
      setTimeout(() => {
        navigate('/admin/pet');
      }, 1000);
    } catch (err) {
      setNotification({ message: 'Không thể tạo pet', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="container mx-auto p-4 bg-white">
      ...
      <h1 className="text-2xl font-bold mb-4">Tạo linh thú mới</h1>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-4">
          ...
          <input
            ...
            required
          />
          {img && <img src={URL.createObjectURL(img)} alt="Pet" className="mt-2 h-24" />}
        </div>
        <div className="mb-4">
